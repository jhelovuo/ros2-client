...
use super::{ServiceMapping, RmwRequestId, request_id};
...
impl<R:Message> RequestWrapper<R> {
  ...
  pub(super) fn unwrap(&self, service_mapping: ServiceMapping, message_info: &MessageInfo) 
...
impl<R:Message> ResponseWrapper<R> {
  ...
  pub(super) fn unwrap(&self, service_mapping: ServiceMapping, message_info: MessageInfo, client_guid:GUID) 
    -> dds::Result<(RmwRequestId, R)> 
  {
    match service_mapping {
      ServiceMapping::Basic => {
...
pub(super) type SimpleDataReaderR<RW> = no_key::SimpleDataReader<RW,ServiceDeserializerAdapter<RW>>;
