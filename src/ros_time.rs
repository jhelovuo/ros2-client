...
impl ROSTime {
  ...
  {
    Self { chrono_time: chrono::Utc::now() }
  }
  ...
  pub const fn from_nanos(nanos_since_unix_epoch: i64) -> Self {
    Self { chrono_time: chrono::DateTime::<Utc>::from_timestamp_nanos(nanos_since_unix_epoch) }
  }
...
impl Sub for ROSTime {
  ...
  fn sub(self, other: ROSTime) -> ROSDuration {
    ROSDuration { diff: self.chrono_time - other.chrono_time }
  }
}
...
impl Sub<ROSDuration> for ROSTime {
    ...
    fn sub(self, other: ROSDuration) -> ROSTime {
      ROSTime{ chrono_time: self.chrono_time - other.diff }
    }
}
...
impl ROSDuration {
  ...
  pub const fn from_nanos(nanos: i64) -> Self {
    ROSDuration{ diff: chrono::TimeDelta::nanoseconds(nanos) }
  }
...
impl TryFrom<Duration> for ROSDuration {
  ...
  fn try_from(std_duration: Duration) -> Result<Self, Self::Error> {
    Ok(ROSDuration{ diff:  chrono::Duration::from_std( std_duration )? })
  }
}
...
impl Add for ROSDuration {
  ...
  fn add(self,other: ROSDuration) -> ROSDuration {
    ROSDuration{ diff: self.diff + other.diff }
  }
}
