...
//
// Steady time should be used onyl when it is necessary, e.g. because of
...
//H
// To get offset to UTC time, use now_with_utc() note that this will change over
// time, latest at the next leap second.
...
impl Time {
  pub fn now() -> Time {
    ...
    Time {
      nano_counter: ts.tv_sec * (10i64.pow(9)) + ts.tv_nsec,
    }
  }
  ...
  // Time cannot implement the Zero trait, because Time does not implement Add
  // (with itself). So we just implement a zero() function.
  pub fn zero() -> Time {
    Time { nano_counter: 0 }
  }
}
...
impl Sub<TimeDiff> for Time {
  ...
  fn sub(self, other: TimeDiff) -> Time {
    Time {
      nano_counter: self.nano_counter - other.as_nanos(),
    }
  }
}
...
impl TimeDiff {
  ...
  pub const fn from_millis(millis: i64) -> TimeDiff {
    TimeDiff {
      nano_diff: millis * 1_000_000,
    }
  }

  pub const fn from_secs(secs: i64) -> TimeDiff {
    TimeDiff {
      nano_diff: secs * 1_000_000_000,
    }
  }
  ...
  pub fn as_duration(self) -> Result<Duration, <u64 as TryFrom<i64>>::Error> {
    u64::try_from(self.nano_diff).map(Duration::from_nanos)
  }
...
impl Add for TimeDiff {
  ...
  fn add(self, other: TimeDiff) -> TimeDiff {
    TimeDiff {
      nano_diff: self.nano_diff + other.nano_diff,
    }
  }
}
