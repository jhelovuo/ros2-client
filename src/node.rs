...
use crate::{
    ...
    service::{
        basic::BasicServiceMapping, cyclone::CycloneServiceMapping,
        enhanced::EnhancedServiceMapping, Client, ClientGeneric, ClientT, Server, ServerGeneric,
        ServerT, Service, ServiceMapping,
    },
};
...
impl Node {
    pub(crate) fn new(
        ...
        let rosout_writer = if options.enable_rosout {
            Some(ros_context.create_publisher(&rosout_topic, None)?)
        } else {
            None
        };
        ...
        S::Request: Clone,
    {
        let inner: Box<dyn ClientT<S>> = match service_mapping {
            ServiceMappings::Basic => Box::new(
                self.create_client_generic::<S, BasicServiceMapping<S>>(service_name, qos)?,
            ),
            ServiceMappings::Enhanced => Box::new(
                self.create_client_generic::<S, EnhancedServiceMapping<S>>(service_name, qos)?,
            ),
            ServiceMappings::Cyclone => Box::new(
                self.create_client_generic::<S, CycloneServiceMapping<S>>(service_name, qos)?,
            ),
        };
        ...
        W: 'static + ServiceMapping<S>,
    {
        ...
        let rq_name = Self::check_name_and_add_prefix(
            "rq/".to_owned(),
            &(service_name.to_owned() + "Request"),
        )?;
        let rs_name = Self::check_name_and_add_prefix(
            "rr/".to_owned(),
            &(service_name.to_owned() + "Reply"),
        )?;
        ...
        ClientGeneric::<S, W>::new(self, &rq_topic, &rs_topic, Some(qos))
    }
        ...
        S::Request: Clone,
    {
        let inner: Box<dyn ServerT<S>> = match service_mapping {
            ServiceMappings::Basic => Box::new(
                self.create_server_generic::<S, BasicServiceMapping<S>>(service_name, qos)?,
            ),
            ServiceMappings::Enhanced => Box::new(
                self.create_server_generic::<S, EnhancedServiceMapping<S>>(service_name, qos)?,
            ),
            ServiceMappings::Cyclone => Box::new(
                self.create_server_generic::<S, CycloneServiceMapping<S>>(service_name, qos)?,
            ),
        };
        ...
        SW: 'static + ServiceMapping<S>,
    {
        ...
        ServerGeneric::<S, SW>::new(self, &rq_topic, &rs_topic, Some(qos))
    }
}
